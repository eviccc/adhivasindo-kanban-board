...
import type { Task, Column, ColumnId, Label, Assignee } from "@/types/task";
import {
  saveTasks,
  loadTasks,
  saveColumns,
...
const DUMMY_TASKS: Task[] = [
  {
    ...
    description:
      "Conduct UX research and gather references for the podcast & video streaming platform redesign.",
  ...
  },
  {
    ...
    description:
      "Fix payment gateway integration issues causing failed transactions on mobile.",
    boardName: DEFAULT_BOARD,
    columnId: "todo",
  ...
  },
  {
    ...
    description:
      "Create low and high-fidelity wireframes for the new company landing page.",
    boardName: DEFAULT_BOARD,
    columnId: "doing",
  ...
  },
  {
    ...
    description:
      "Evaluate and configure HubSpot CRM for the marketing team's daily workflows.",
    boardName: DEFAULT_BOARD,
    columnId: "doing",
  ...
  },
  {
    ...
    description:
      "Iterate on logo concepts and finalize brand identity guidelines.",
    boardName: DEFAULT_BOARD,
    columnId: "review",
  ...
  },
  {
    ...
    description:
      "Design and export a consistent set of UI icons in SVG format.",
    boardName: DEFAULT_BOARD,
    columnId: "review",
  ...
  },
  {
    ...
    description:
      "Build reusable email template components with responsive design.",
    boardName: DEFAULT_BOARD,
    columnId: "done",
    assignees: [DUMMY_ASSIGNEES[0], DUMMY_ASSIGNEES[1]],
    dueDate: "2025-08-20",
  ...
  },
  {
    ...
    description:
      "Redesign the blog editor UX and build the new playlist browsing experience.",
    boardName: DEFAULT_BOARD,
    columnId: "rework",
  ...
  },
  {
    ...
    description:
      "Prepare onboarding materials and schedule 3-day orientation for Q3 new hires.",
    boardName: DEFAULT_BOARD,
    columnId: "rework",
    assignees: [DUMMY_ASSIGNEES[1], DUMMY_ASSIGNEES[4]],
    dueDate: "2025-08-09",
...
export const useTaskStore = defineStore("tasks", () => {
  ...
  const tasks = ref<Task[]>(migratedTasks);
  const columns = ref<Column[]>(loadColumns() ?? DEFAULT_COLUMNS);
  const columnOrder = ref<ColumnId[]>(
    loadColumns()?.map((c: Column) => c.id) ?? DEFAULT_COLUMNS.map((c) => c.id)
  );

  // Filters
  const searchQuery = ref("");
  const filterAssignee = ref<string | null>(null);
  const filterLabel = ref<Label | null>(null);
  const filterDueDate = ref<"overdue" | "today" | "this_week" | null>(null);
  ...
  const allAssignees = computed<Assignee[]>(() => {
  ...
  const filteredTasks = computed(() => {
    return tasks.value.filter((task) => {
      ...
      if (
        ...
        return false;
      if (filterLabel.value && task.label !== filterLabel.value) return false;
  ...
  function reorderTasks(columnId: ColumnId, newOrder: Task[]) {
